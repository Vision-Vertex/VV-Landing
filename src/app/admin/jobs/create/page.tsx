...
export default function CreateJobPage() {
  ...
  // --- Reusable Classes ---
  const inputClasses = "w-full h-10 px-3 bg-white border border-gray-200 rounded-md text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all";
  const textareaClasses = "w-full px-3 py-2.5 bg-white border border-gray-200 rounded-md text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all resize-none";
  const labelClasses = "text-gray-700 font-medium mb-1.5 block text-sm";
  const errorClasses = "text-red-500 text-sm mt-1 flex items-center gap-1";
  const selectTriggerClasses = "w-full h-10 px-3 bg-white border border-gray-200 rounded-md text-sm text-gray-900 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all";
  const sectionIconClass = "w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center shrink-0";
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-8 space-y-6">
        <AdminPageHeader
          ...
          description="Fill in the details below to create a new job posting. Fields marked with * are required."
        />
        ...
        {error && (
          <div className="p-4 bg-red-50 border border-red-200 rounded-md flex items-start gap-3">
            ...
            <div>
              <span className="text-red-800 font-medium block text-sm">Error</span>
              <span className="text-red-700 text-sm">{error.message}</span>
              <button onClick={reset} className="text-red-600 text-sm font-medium mt-1 hover:underline">
                Dismiss
              </button>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pb-8">
          ...
          <div className="bg-white rounded-md p-6 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3 mb-5 pb-4 border-b border-gray-100">
              ...
              <h2 className="text-lg font-bold text-gray-900">Skills & Compensation</h2>
            </div>
          ...
          <div className="bg-white rounded-md p-6 shadow-sm border border-gray-100">
            <div className="flex items-center gap-3 mb-5 pb-4 border-b border-gray-100">
              ...
              <h2 className="text-lg font-bold text-gray-900">Publication Status</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {statuses.map((status) => (
            ...
            <p className="text-sm text-gray-500 mt-3"><strong>Draft:</strong> Save later | <strong>Active:</strong> Publish now | <strong>Closed:</strong> Stop applications</p>
          </div>

          <div className="flex flex-col sm:flex-row gap-3 justify-end pt-2">
            <Button type="button" variant="outline" onClick={() => router.push('/admin/jobs')} disabled={isPending}>
              <X size={16} className="mr-2" /> Cancel
            </Button>
            <Button type="submit" disabled={isPending}>
              {isPending ? <><Loader2 size={16} className="mr-2 animate-spin" /> Creating...</> : <><Save size={16} className="mr-2" /> Create Job Posting</>}
            </Button>
          </div>
        </form>
      </div>
